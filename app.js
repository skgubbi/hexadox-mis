...
/* Edit Submitted Data: use explicit DOM references for saving so browser global IDs cannot break the handler. */
if(location.pathname.endsWith("/edit-data.html")||location.pathname.endsWith("edit-data.html")){
 window.saveSales=async function(){
  const header=window.currentHeader;
  const b=document.getElementById("saveButton");
  const msg=document.getElementById("message");
  if(!header){if(typeof showMessage==="function")showMessage("Please load submitted sales first.","error");return;}
  if(!b)return;
  ...
  try{
   ...
   if(!rows.length)throw new Error("No sales rows found to save.");
   for(const u of rows){
    const{id,...payload}=u;
    const pts=Number(payload.pts)||0,pu=Number(payload.primary_units)||0,su=Number(payload.secondary_units)||0;
    payload.primary_value=Number((pts*pu).toFixed(2));payload.secondary_value=Number((pts*su).toFixed(2));
    const result=await client.from("sales_entry_details").update(payload).eq("id",id).eq("entry_id",header.id).select("id,pts,primary_units,primary_value,secondary_units,secondary_value");
    if(result.error)throw result.error;
    if(!result.data||!result.data.length)throw new Error("Database did not update product row "+id+". Check the Supabase update policy.");
    const saved=result.data[0];
    if(payload.pts!==null&&Math.abs(Number(saved.pts)-Number(payload.pts))>0.000001)throw new Error("PTS was not retained for product row "+id+".");
   }
   ...
   if(typeof loadSales==="function"&&typeof setLoading==="function")await loadSales();
